...
export default function SignInModal({ open, onClose, onShopCategorySelect }) {
  ...
  const calculateDates = (dayName) => {
    ...
    const nextDeliveryStr = nextDeliveryDate.toLocaleDateString(
      "en-GB",
      options,
    );
  ...
  const handleCheckPincode = async () => {
    ...
    try {
      const response = await fetch(
        `${window.ENV.BACKEND_API}/api/pincode/check`,
        {
          method: "POST",
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const response = await fetch(
        `${window.ENV.BACKEND_API}/api/user/send-otp`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await fetch(
        `${window.ENV.BACKEND_API}/api/user/login-otp`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleForgotSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `${window.ENV.BACKEND_API}/api/user/forgot-password`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  if (available && deliveryData) {
    ...
    return (
      <div className="modal-overlay" onClick={onClose}>
        <div
          className="modal-container wide-modal"
        ...
        >
          ...
          <div className="success-view">
            <h2 className="delivery-header">
              <span className="delivery-date-highlight">
            ...
            <p className="delivery-message">
              We deliver to your area every {deliveryData.deliveryDay}. The next
              time we can deliver to you is on{" "}
              <strong>{deliveryData.nextDelivery}</strong>.
            </p>

            <div className="cutoff-message">
              You’ve got plenty of time to use up anything in the fridge and
              start meal planning before your last chance to order at{" "}
              <strong>
                11:45pm on {deliveryData.cutoffDay} {deliveryData.cutoffDate}
              </strong>
              .
            </div>
            ...
            <div className="build-order-section">
              ...
              <p className="build-order-desc">
                Pick whatever items you fancy from our full range, or choose one
                of our set boxes and top up with any extras. You decide what you
                want, and how often - no subscription necessary.
              </p>

              <div className="category-grid">
                <div
                  onClick={() => handleCategoryClick("Fresh Fruits")}
                ...
                <div
                  onClick={() => handleCategoryClick("Leafy & others")}
                  className="category-card"
                >
                  Leafy & others
                </div>
                <div
                  onClick={() => handleCategoryClick("What's new")}
                  className="category-card"
                >
                  What's new
                </div>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-container" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-left">
          ...
          {inputType !== "phone" && !forgotView && (
            <>
              ...
              <button className="small-btn" onClick={handleEmailLogin}>
                Sign in
              </button>
              <p className="forgot-link" onClick={() => setForgotView(true)}>
                Forgotten password?
              </p>
            </>
          )}
          ...
          {forgotView && (
            <div className="forgot-view">
              ...
              <p className="forgot-link" onClick={() => setForgotView(false)}>
                Back to Sign in
              </p>
            </div>
          )}
          ...
          {inputType === "phone" && !isOtpSent && !forgotView && (
            <button className="small-btn" onClick={handleSendOtp}>
              Get OTP
            </button>
          )}
          ...
          {inputType === "phone" && isOtpSent && !forgotView && (
            <>
              <label className="label-text" style={{ marginTop: "15px" }}>
                OTP
              </label>
              <input
                className="input-box"
              ...
              />
              <button className="small-btn" onClick={handleVerifyOtp}>
                Verify & Sign in
              </button>
              <p
        ...
        <div className="modal-right">
          ...
          <h3 className="modal-title">New to Organic Vibes?</h3>
          <div className="underline"></div>
